...
class TestDataExtraction(unittest.TestCase):
    ...
    def test_parse_ranking_xml(self):
        ...
        shapes = [(318, 7), (95, 7)]
        values = [("nakt", 5, 501, 501, 417, 80, 0), ("nakt", 1, 8156, 404, 2057, 1413, 4434)]
        ...
        for file, shape, value in zip(files, shapes, values):
            with self.subTest(file):
                ...
                self.assertEqual(tuple(df.loc[35564].values), value)

    def test_extract_ranking_from_html(self):
        df = extract_ranking_from_html(self.resource("test_middle.html"))
        self.assertEqual(
            tuple(df.loc[35564].values),
            (6645, "nakt", "★04", "-", "HARD", "AAA", 3608, 4012, "89.93%",
        ...
        for file, size in zip(files, sizes):
            with self.subTest(file=file):
                ...
                self.assertEqual(df.shape, (size, 22))
